...
export function HeroSection() {
  ...
  return (
    ...
    >
      ...
      {/* Mobile Neumorphic Hero Layout (Updated) */}
      ...
      <div className="hidden lg:flex flex-col w-full z-20 pb-0 mt-12 xl:mt-14 items-center">
        <div className="w-full max-w-[1040px] mx-auto px-6 lg:px-8">
          
          <div className="grid grid-cols-3 grid-rows-2 gap-6 xl:gap-7 items-stretch mb-8">
            
            {/* ROW 1, COL 1 & ROW 2, COL 1: 1st Column Vertical Stack */}
            <div className="row-span-2 flex flex-col h-full gap-6 xl:gap-7">
              ...
              {/* Skills Card (Flat/Compact) */}
              <div className="flex-1 rounded-[40px] bg-white/90 dark:bg-zinc-950/90 border border-zinc-200/50 dark:border-zinc-800/80 p-6 xl:p-8 flex flex-col shrink-0 overflow-hidden shadow-none">
                <h3 className="text-[1.1rem] xl:text-[1.25rem] font-bold text-zinc-900 dark:text-white mb-4 text-center">Skills</h3>
                <div className="flex-1 flex items-center justify-center">
                  <div className="grid grid-cols-3 gap-y-7 gap-x-4 w-full">
                    {heroSkills.slice(0, 12).map((skill, i) => (
                      <div key={i} className="flex flex-col items-center gap-1.5 opacity-80 hover:opacity-100 transition-opacity">
                        <div className="w-8 h-8 xl:w-9 xl:h-9 relative grayscale dark:invert-[0.15]">
                          <Image src={skill.logoUrl} alt={skill.name} fill className="object-contain" unoptimized />
                        </div>
                        <span className="text-[0.6rem] xl:text-[0.65rem] font-semibold text-zinc-600 dark:text-zinc-400 text-center leading-tight tracking-wider">{skill.name}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="w-full relative aspect-[4/5] neu-container overflow-hidden rounded-[40px] xl:rounded-[40px] grid grid-rows-[40%_20%_20%_20%] shrink-0 min-h-0">
                ...
                <div className="grid grid-cols-4">
                  ...
                  <a href={socialLinks.github} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center border-r neu-separator group hover:bg-zinc-100/10 transition-colors">
                    <div className="w-12 h-12 xl:w-14 xl:h-14 rounded-full flex items-center justify-center transition-colors text-zinc-900 dark:text-zinc-100 group-hover:opacity-100 opacity-90 group-hover:scale-110">
                  ...
                  <a href={socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center group hover:bg-zinc-100/10 transition-colors">
                    <div className="w-12 h-12 xl:w-14 xl:h-14 rounded-full flex items-center justify-center transition-colors text-[#0A66C2] dark:text-[#5DA9FF] group-hover:opacity-100 opacity-90 group-hover:scale-110">
                      <svg viewBox="0 0 24 24" className="block w-6 h-6 xl:w-7 xl:h-7 fill-current" xmlns="http://www.w3.org/2000/svg">
                        <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                      </svg>
                    </div>
            ...
            {/* ROW 1, COL 3 & ROW 2, COL 3: 3rd Column Vertical Stack */}
            <div className="row-span-2 flex flex-col h-full gap-6 xl:gap-7">
              {/* Featured Project 2 (Square) */}
              <div className="w-full relative aspect-square rounded-[40px] xl:rounded-[40px] overflow-hidden neu-raised shadow-none border-none shrink-0 pointer-events-none select-none">
                <Image 
              ...
              {/* Unified Bio & Education Stack */}
              <div className="flex-1 flex flex-col items-stretch">
                <div className="h-full rounded-[40px] bg-white/90 dark:bg-zinc-900/30 border border-zinc-200/50 dark:border-zinc-800/50 p-6 xl:p-8 flex flex-col justify-between">
                  {/* Bio Content */}
                  <div className="space-y-4">
                    <h3 className="text-xl xl:text-[1.4rem] font-bold leading-[1.2] text-zinc-900 dark:text-white tracking-tight">
                      Bridging design <br/>& engineering.
                    </h3>
                    <p className="text-sm xl:text-[0.95rem] font-medium leading-[1.6] text-zinc-600 dark:text-zinc-400">
                      I craft high-performance apps & AI interfaces with technical precision and aesthetic excellence. My work bridges the gap between complex engineering and human-centered design, focusing on creating products that are intuitive, functional, and visually stunning.
                    </p>
                  </div>

                  {/* Integrated Education Status */}
                  <div className="mt-8 pt-6 border-t border-zinc-200/60 dark:border-zinc-800/50">
                    <div className="flex items-center gap-2 mb-2.5">
                      <div className="relative">
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-glow-pulse" />
            ...
            {/* ROW 2, COL 2: Profile Card (Centered) */}
