        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          {[
            ...
            <Link
              ...
              href={item.href}
              className={cn(
                "group relative rounded-2xl border transition-all duration-300 overflow-hidden h-24 sm:h-28 md:h-56 lg:h-72 xl:h-80",
                item.customBg || "bg-gray-100 dark:bg-card/60 backdrop-blur-sm hover:bg-gray-200/80 dark:hover:bg-card/80",
                item.borderColor
              )}
            >
              {/* Gradient background */}
              ...
              <ArrowUpRight className={cn("md:hidden absolute top-3 right-3 w-4 h-4 transition-colors z-10", item.iconColor || "text-muted-foreground/70 group-hover:text-foreground")} />
              <div className="absolute inset-0 flex items-end px-4 pb-5 md:pb-5 md:pl-5 z-10">
                <div>
                  <span className={cn("text-sm sm:text-base font-semibold transition-colors leading-tight block", item.titleColor || "text-foreground group-hover:text-foreground")}>
                    {item.label}
                  </span>
                  <span className={cn("hidden md:block text-xs transition-colors mt-1", item.accentColor || "text-muted-foreground/70 group-hover:text-muted-foreground")}>
                    {item.accent}
                  </span>
                </div>
              </div>
